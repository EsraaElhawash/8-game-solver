...
def AStarSearchManhattan(StartState,Z):
    ...
    Puzzle= artintelgame.Game(StartState,Z)
    q=[Puzzle]
    heapq.heapify(q)
...
def WriteInFile(state,i,string,run_time,depth):
    ...
    with open("ArtificialIntelligence1.txt","a") as file1:    
     for f in fofa:
...
def main():
    f = open('ArtificialIntelligence1.txt', 'r+')
    f.truncate(0)    
